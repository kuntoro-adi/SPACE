...
def train(model, num_epochs, dataloaders, device, optimizer, warmup_scheduler, lr_scheduler, save_name='results/file_result'):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and running_loss[0].item() < best_loss:
                ...
                torch.save(best_model_wts, save_name + '.pth')
                print()
        ...
        loss_hist.sort_values(by=['phase', 'epoch']).to_csv(save_name + '_sm.csv', index=False)
        print()
...
train_img_path = "../Datasets/salicon/images/train/"
train_msk_path = "../Datasets/salicon/maps/train/"
train_fix_path = "../Datasets/salicon/fixations/train/"
val_img_path = "../Datasets/salicon/images/val/"
val_msk_path = "../Datasets/salicon/maps/val/"
val_fix_path = "../Datasets/salicon/fixations/val/"
...
parser.add_argument('--save_name', action='store', type=str, help="File name to save.", default='results/space_model')
...
if __name__ == "__main__":
    ...
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    train_ds = ISFDataset(args.train_img_path, args.train_msk_path, args.train_fix_path, resize=(640,480), transform=_default_transform)
    ...
    train(model, num_epochs, dataloaders, device, optimizer, warmup_scheduler, lr_scheduler, save_name=args.save_name)
    print('Finished.')